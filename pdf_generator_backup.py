...
from utils import number_to_words_indian, format_currency
...
class InvoicePDFGenerator:
    ...
    def __init__(self):
        # Default template path - will be determined based on invoice type
        self.gst_template_path = os.path.join(os.path.dirname(__file__), 'templates', 'invoice.html')
        self.non_gst_template_path = os.path.join(os.path.dirname(__file__), 'templates', 'invoice_non_gst.html')
    ...
    def prepare_template_data(self, invoice_data):
        ...
        invoice_type = invoice.get('type', 'GST')
        
        # Use different preparation based on invoice type
        if invoice_type and invoice_type.upper() in ['NON-GST', 'NON GST', 'NONGST']:
            return self.prepare_non_gst_template_data(invoice_data)
        else:
            return self.prepare_gst_template_data(invoice_data)
    
    def prepare_non_gst_template_data(self, invoice_data):
        """Prepare data for Non-GST HTML template rendering"""
        invoice = invoice_data['invoice']
        ...
        subtotal_net = total_amount - total_discount
        grand_total = float(invoice.get('grand_total', subtotal_net))
        round_off = grand_total - subtotal_net
        
        # Amount in words
        amount_words = number_to_words_indian(grand_total)
        
        # Template data for Non-GST invoice
        template_data = {
            # Invoice details
            'invoice_no': invoice.get('invoice_no', ''),
            'invoice_date': formatted_date,
            'due_date': formatted_date,  # Can be modified if due_date exists in invoice
            ...
            'company_pincode': '390001',
            'company_phone': company.get('phone', ''),
            'company_email': company.get('email', ''),
            
            # Payment terms
            'payment_terms': 'Cash',
            'transportation_mode': 'By Hand',
            'po_number': invoice.get('po_number', ''),
            
            # Bill to / Ship to (same as party for now)
            'bill_to_name': party.get('name', ''),
            'bill_to_address': party.get('address', ''),
            'bill_to_city': party.get('city', ''),
            'bill_to_state': party.get('state', ''),
            'bill_to_pincode': party.get('pincode', ''),
            'bill_to_phone': party.get('phone', ''),
            
            'ship_to_name': party.get('name', ''),
            'ship_to_address': party.get('address', ''),
            'ship_to_city': party.get('city', ''),
            'ship_to_state': party.get('state', ''),
            'ship_to_pincode': party.get('pincode', ''),
            'ship_to_phone': party.get('phone', ''),
            ...
            'subtotal_net': f"{subtotal_net:.2f}",
            'net_amount_before_round': f"{subtotal_net:.2f}",
            'round_off': f"{round_off:.2f}" if abs(round_off) > 0.01 else "0.00",
            'final_amount': f"{grand_total:.2f}",
            
            # Additional
            'additional_notes': invoice.get('notes', ''),
            ...
            # Terms
            'terms_conditions': company.get('terms', 'All disputes subject to local jurisdiction only.')
        }
        
        return template_data
    
    def prepare_gst_template_data(self, invoice_data):
        """Prepare data for GST HTML template rendering"""
        invoice = invoice_data['invoice']
        party = invoice_data['party'] or {}
        items = invoice_data['items']
        company = invoice_data['company']
        ...
        # Extract buyer details for template
    ...
    def render_html_template(self, template_data):
        ...
        try:
            ...
            # Add empty rows for better layout (like the original)
            for i in range(max(0, 8 - len(template_data['items']))):
                items_html += """
                    <tr>
